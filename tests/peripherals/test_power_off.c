...
int main(void)
{
   ...
   system_enter_power_off_mode(wakeup_pin, (wake_criteria == WAKE_WITH_RTC) ? rtc_get_timestamp() + 5 : 0, true);
   ...
   // Enter power-down mode until awoken 5 seconds in the future (or by a GPIO pin level change)
   system_enter_power_off_mode(wakeup_pin, (wake_criteria == WAKE_WITH_RTC) ? rtc_get_timestamp() + 5 : 0, false);
   system_reset();
